...
```{r}
# The idea here is to find some tunning parameter that can seet the upper bound of the xlim
# I haven't find one yet
#real_V3 <- quantile(V3$real)[4]
```

```{r}
# Here I set 0.5 as the upper bound of the xlim for this particular dataset
# It is not efficient
...
Here, since the signal- 0*1, for this particular dataset, the signal identifier pull a number of spefici value.

```{r}
type4
```

We can combine the history dataset with the system log file

```{r}
combine <- cbind(history_sep, pairwise)
```

From here, we can use the combine dataset to do all kinds of operation like finding patterns of severl query type, find query taht has largest run time of real, user, and sys and analyze these query.

```{r}

#Wall-Clock Time
# Find the row index with the max value in real-V3
max_real_V3_idex <- which.max(combine$V3_real)
# Find the row index with the max value in real-V4.C
max_real_V4.C_idex <- which.max(combine$V4.C_real)
# Find the row index with the max value in real-V4.M
max_real_V4.C_idex <- which.max(combine$V4.M_real)

# CPU Run Time
# Find the row index with the max value in real-V3
max_cpu_V3_idex <- which.max(combine$V3_user)
# Find the row index with the max value in real-V4.C
max_cpu_V4.C_idex <- which.max(combine$V4.C_user)
# Find the row index with the max value in real-V4.M
max_cpu_V4.M_idex <- which.max(combine$V4.M_user)


# Kernel Run Time
# Find the row index with the max value in real-V3
max_ker_V3_idex <- which.max(combine$V3_sys)
# Find the row index with the max value in real-V4.C
max_ker_V4.C_idex <- which.max(combine$V4.C_sys)
# Find the row index with the max value in real-V4.M
max_ker_V4.M_idex <- which.max(combine$V4.M_sys)
```

```{r}
# Subset the data frame to get the row with the max value in V3_real
max_query_V3_real <- combine[max_real_V3_idex, ]
# Subset the data frame to get the row with the max value in V3_real
max_query_V4.C_real <- combine[max_real_V4.C_idex, ]
# Subset the data frame to get the row with the max value in V3_real
max_query_V4.M_real <- combine[max_real_V4.C_idex, ]

# Subset the data frame to get the row with the max value in V3_real
max_query_V3_cpu <- combine[max_cpu_V3_idex, ]
# Subset the data frame to get the row with the max value in V3_real
max_query_V4.C_cpu <- combine[max_cpu_V4.C_idex, ]
# Subset the data frame to get the row with the max value in V3_real
max_query_V4.M_cpu <- combine[max_cpu_V4.M_idex, ]


# Subset the data frame to get the row with the max value in V3_real
max_query_V3_ker <- combine[max_ker_V3_idex, ]
# Subset the data frame to get the row with the max value in V3_real
max_query_V4.C_ker <- combine[max_ker_V4.C_idex, ]
# Subset the data frame to get the row with the max value in V3_real
max_query_V4.M_ker <- combine[max_ker_V4.M_idex, ]


# Building a dataset that contains the ma real, user, and sys run time among three systems
max_query_df <- rbind(max_query_V3_real, max_query_V4.C_real, max_query_V4.M_real,
                      max_query_V3_cpu, max_query_V4.C_cpu, max_query_V4.M_cpu,
                      max_query_V3_ker, max_query_V4.C_ker, max_query_V4.M_ker)
max_query_df
```

From the table, one thing we find interesting is that for max cpu run time seems linked quite tightly with time-0xRANGE.


This way of exploring may not make much sense. Another way of analyzing this is based on the the frequency of each type of query and do


# Select the 5% percent of the shortest real run time

```{r}
# Get the number of rows in the data frame
n <- nrow(combine)
# Select the smallest 5% of values for column 'value'
# I should be able to write a for loop from 7th colmn to the end

#V3
V3_real_smallest <- combine %>% 
  slice_min(V3_real, n = floor(0.05 * n))
V3_real_smallest <- V3_real_smallest[, 1:4]
V3_user_smallest <- combine %>% 
  slice_min(V3_user, n = floor(0.05 * n))
V3_user_smallest <- V3_user_smallest[, 1:4]
V3_sys_smallest <- combine %>% 
  slice_min(V3_sys, n = floor(0.05 * n))
V3_sys_smallest <- V3_sys_smallest[, 1:4]


#V4.C
V4.C_real_smallest <- combine %>% 
  slice_min(V4.C_real, n = floor(0.05 * n))
V4.C_real_smallest <- V4.C_real_smallest[, 1:4]
V4.C_user_smallest <- combine %>% 
  slice_min(V4.C_user, n = floor(0.05 * n))
V4.C_user_smallest <- V4.C_user_smallest[, 1:4]
V4.C_sys_smallest <- combine %>% 
  slice_min(V4.C_sys, n = floor(0.05 * n))
V4.C_sys_smallest <- V4.C_sys_smallest[, 1:4]



#V4.M
V4.M_real_smallest <- combine %>% 
  slice_min(V4.M_real, n = floor(0.05 * n))
V4.M_real_smallest <- V4.M_real_smallest[, 1:4]
V4.M_user_smallest <- combine %>% 
  slice_min(V4.M_user, n = floor(0.05 * n))
V4.M_user_smallest <- V4.M_user_smallest[, 1:4]
V4.M_sys_smallest <- combine %>% 
  slice_min(V4.M_sys, n = floor(0.05 * n))
V4.M_sys_smallest <- V4.M_sys_smallest[, 1:4]

```

```{r}
# Define a function called type_freq
# Find the frequency of query type in 5% smallest run time
#input: dataframe
type_freq <- function(df) {
  freq_tables <- list() # initialize an empty list to store frequency tables for each column
  ...
  for (i in 1:ncol(df)) {
    ...
    # Assign the frequency table to the list with the column name as the list element name
    freq_tables[[col_name]] <- freq_table
    
  }
  
  # Print the frequency tables for each column
  print(freq_tables)
}
```
...
```{r}
type_freq(V3_real_smallest)
```

```{r}
type_freq(V4.C_real_smallest)
```

```{r}
type_freq(V4.M_real_smallest)
```
# I know this is a list, but I can't subract the value in this list. Therefore, I can't do visulization to this.

This is meaningful since we choose smallest 5% of data for each system and for each run time. Then, we can find the corresponding query type to see what is unique or what is the most frequent to maybe find something interesting.

#############################################Work Done Here##########################################################


There are certainly a lot of things I can futher analysis. I believe once I have the stucture of the scheme or the return column, I can do more.
